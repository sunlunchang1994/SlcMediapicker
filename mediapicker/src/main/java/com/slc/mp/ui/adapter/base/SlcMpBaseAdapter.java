 ...
 * author：on the way
 */
public abstract class SlcMpBaseAdapter<T> extends RecyclerView.Adapter<SlcMpBaseViewHolder> {
    ...
    public SlcMpBaseAdapter(@NonNull Context context, @NonNull List<T> date) {
    ...
    public SlcMpBaseAdapter(@NonNull Context context, @NonNull List<T> date, @LayoutRes int layoutId) {
        this(context, date);
        addLayoutByType(0, layoutId);
    }

    protected void addLayoutByType(int type, @LayoutRes int layoutId) {
        mLayoutArray.put(type, layoutId);
    }
    ...
    @NonNull
    @Override
    public SlcMpBaseViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
        SlcMpBaseViewHolder slcMpBaseViewHolder;
        switch (viewType) {
            case HEADER_VIEW:
                slcMpBaseViewHolder = createBaseViewHolder(mHeaderLayout);
                break;
            case FOOTER_VIEW:
                slcMpBaseViewHolder = createBaseViewHolder(mFooterLayout);
                break;
            default:
                slcMpBaseViewHolder = onCreateDefViewHolder(viewGroup, viewType);
                bindViewClickListener(slcMpBaseViewHolder);
        }
        slcMpBaseViewHolder.setAdapter(this);
        return slcMpBaseViewHolder;
    }

    protected SlcMpBaseViewHolder onCreateDefViewHolder(ViewGroup parent, int viewType) {
        return new SlcMpBaseViewHolder(mLayoutInflater.inflate(mLayoutArray.get(viewType), parent, false));
    }

    /**
     * if you want to use subclass of SlcMpBaseViewHolder in the adapter,
    ...
    protected SlcMpBaseViewHolder createBaseViewHolder(View view) {
        Class temp = getClass();
        Class z = null;
        ...
        // 泛型擦除会导致z为null
        if (z == null) {
            slcMpBaseViewHolder = new SlcMpBaseViewHolder(view);
        } else {
            slcMpBaseViewHolder = createGenericKInstance(z, view);
        }
        return slcMpBaseViewHolder != null ? slcMpBaseViewHolder : new SlcMpBaseViewHolder(view);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    private SlcMpBaseViewHolder createGenericKInstance(Class z, View view) {
        try {
            ...
            if (z.isMemberClass() && !Modifier.isStatic(z.getModifiers())) {
                ...
                return (SlcMpBaseViewHolder) constructor.newInstance(this, view);
            } else {
                constructor = z.getDeclaredConstructor(View.class);
                constructor.setAccessible(true);
                return (SlcMpBaseViewHolder) constructor.newInstance(view);
            }
        ...
        if (type instanceof ParameterizedType) {
            ...
            for (Type temp : types) {
                if (temp instanceof Class) {
                    ...
                    if (SlcMpBaseViewHolder.class.isAssignableFrom(tempClass)) {
                        return tempClass;
                    }
                } else if (temp instanceof ParameterizedType) {
                    Type rawType = ((ParameterizedType) temp).getRawType();
                    if (rawType instanceof Class && SlcMpBaseViewHolder.class.isAssignableFrom((Class<?>) rawType)) {
    ...
    @Override
    public void onBindViewHolder(@NonNull SlcMpBaseViewHolder slcMpBaseViewHolder, int position) {
        int viewType = slcMpBaseViewHolder.getItemViewType();
        switch (viewType) {
            case HEADER_VIEW:
                break;
            case FOOTER_VIEW:
            ...
            case 0:
            default:
                convert(slcMpBaseViewHolder, getItem(position - getHeaderLayoutCount()));
                break;
        }
    }
     ...
     */
    protected abstract void convert(SlcMpBaseViewHolder helper, T item);


    private void bindViewClickListener(final SlcMpBaseViewHolder slcMpBaseViewHolder) {
        if (slcMpBaseViewHolder == null) {
            return;
        }
        final View view = slcMpBaseViewHolder.itemView;
        if (view == null) {
            return;
        }
        if (getOnItemClickListener() != null) {
            view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    setOnItemClick(v, slcMpBaseViewHolder.getLayoutPosition() - getHeaderLayoutCount());
                }
            });
        }
        if (getOnItemLongClickListener() != null) {
            view.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    return setOnItemLongClick(v, slcMpBaseViewHolder.getLayoutPosition() - getHeaderLayoutCount());
                }
            });
        }
    }
    ...
    public interface OnItemChildClickListener {
         ...
         */
        void onItemChildClick(SlcMpBaseAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemChildLongClickListener {
         ...
         */
        boolean onItemChildLongClick(SlcMpBaseAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemLongClickListener {
         ...
         * @return true if the callback consumed the long click ,false otherwise
         */
        boolean onItemLongClick(SlcMpBaseAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemClickListener {
         ...
         */
        void onItemClick(SlcMpBaseAdapter adapter, View view, int position);
    }
